...
class GoogleCalendarApi { 
    ...
    // Constructor to initialize the class with parameters if provided
    function __construct($params = array()) { 
        if (count($params) > 0){ 
            $this->initialize($params);         
        } 
    } 
    // Method to initialize class properties with provided parameters
    ...
    // access token from Google using the provided authorization code
    ...
    // retrieving the user's calendar timezone using the access token
    ...
    // retrieving the list of calendars accessible by the user using the access token
    public function GetCalendarsList($access_token) { 
        // URL parameters for the calendar list request
    ...
    // creating a calendar event using the access token
    public function CreateCalendarEvent($access_token, $calendar_id, $event_data, $all_day, $event_datetime, $event_timezone) { 
        ...
        if($all_day == 1){ 
            ...
            // For timed events, prepare the dateTime with timezone offset
    ...
    // getting the timezone offset in the format +HH:MM or -HH:MM
